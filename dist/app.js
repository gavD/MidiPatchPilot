...
const LFO_MAX_TOTAL_SENDS_PER_SECOND = Math.floor((LFO_CLASSIC_MIDI_BAUD / LFO_BITS_PER_CONTROL_CHANGE) * LFO_BANDWIDTH_HEADROOM);
...
function renderInlineLfoControls(control) {
    ...
    const depth = createLfoRange("LFO Depth", 0, maxLfoDepth(control), 1, lfo.depth, `${control.label} LFO depth`, depthValue);
    depth.input.dataset.lfoControl = "depth";
    depthValue.dataset.lfoOutput = "depth";
    const rateValue = document.createElement("output");
    const rate = createLfoRange("LFO Rate", LFO_MIN_RATE_HZ, LFO_MAX_RATE_HZ, 0.1, lfo.rate, `${control.label} LFO rate`, rateValue);
...
function getLfoViewState(control) {
    const lfo = state.lfos.get(String(control.cc));
    return {
...
function calculateLfoTiming(control, lfo, activeCount) {
    ...
    const perControlBudget = Math.max(LFO_MIN_CONTROL_SENDS_PER_SECOND, Math.floor(LFO_MAX_TOTAL_SENDS_PER_SECOND / Math.max(1, activeCount)));
...
function normalizeLfoWaveform(value) {
    return lfoWaveformOptions.some((option) => option.value === value) ? value : "triangle";
}
...
function highlightControl(wrapper) {
    ...
    state.highlightTimers.set(cc, window.setTimeout(() => {
...
function isControlType(type) {
    return allowedControlTypes.has(type);
}
